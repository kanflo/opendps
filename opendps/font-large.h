/** Font generated from gfx/Ubuntu-C.ttf 48pt */

#ifndef __FONT_LARGE_H__
#define __FONT_LARGE_H__

#include <stdint.h>

#define FONT_LARGE_MAX_GLYPH_HEIGHT (35)
#define FONT_LARGE_MAX_GLYPH_WIDTH  (22)
#define FONT_LARGE_MAX_DIGIT_WIDTH  (17)
#define FONT_LARGE_DOT_WIDTH        (5)
#define FONT_LARGE_SPACING          (4)
#define FONT_LARGE_SPACE_WIDTH      (9)

extern const uint32_t font_large_height;
extern const uint32_t font_large_num_glyphs;
extern const uint8_t font_large_widths[96];
extern const uint8_t font_large_sizes[96];
extern const uint16_t font_large_offsets[96];
extern const uint8_t font_large_pixdata[1745];

#endif // __FONT_LARGE_H__